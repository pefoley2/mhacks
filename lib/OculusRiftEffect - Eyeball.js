 ...
 */
THREE.OculusRiftEffect.prototype.render = function(scene, camera, vrstate, angle) {
  ...
  if (vrstate) {
	...
	//var eul = new THREE.Vector3(0, -angle * Math.PI / 180, 0);
	//rotMat2.setRotationFromEuler(eul);
	
	
	var point = new THREE.Vector3( 1, 0, 0 );
	point.applyMatrix4(rotMat);
	var angle = Math.atan2(point.z, point.x) * 180 / Math.PI;
	//if (Math.abs(point.x) > 0.33)	{
		//if (point.x > 0) rotateDirection = 1; else rotateDirection = -1;
	//} else	{
		//rotateDirection = 0;
	//}
	
	//angle += rotateDirection * 2;
	document.getElementById('info').innerHTML = angle;
  }
